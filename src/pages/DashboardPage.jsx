...
const DashboardPage = () => {
  ...
  const handleItemRequest = (e, date) => {
    ...
    axios
      ...
      .then((response) => {
        if (response.data && response.data.Data && response.data.Data.item && response.data.Data.item.skuList) {
  ...
  return (
    <div style={{ margin: "20px", backgroundColor: "white", borderRadius: "10px" }}>
            ...
            <div>
              <div className="dropdown_table">
                <div className="dropdown_item">
                  <button type="submit" onClick={(e) => {setDay(7); setSelectedDate(responseData[0]?.date)}}>
                    7 Day
                  </button>
                </div>
                <div className="dropdown_item">
                  <button type="submit" onClick={(e) => {setDay(14); setSelectedDate(responseData[0]?.date)}}>
                    14 Day
                  </button>
                </div>
                <div className="dropdown_item">
                  <button type="submit" onClick={(e) => {setDay(30); setSelectedDate(responseData[0]?.date)}}>
                    30 Day
                  </button>
                </div>
                <div className="dropdown_item">
                  <button type="submit" onClick={(e) => {setDay(60); setSelectedDate(responseData[0]?.date)}}>
                    60 Day
                  </button>
                </div>
              </div>
            ...
            <div style={{ padding: "20px" }} key={index}>
              <div
                ...
                style={{
                  ...
                  width: "50px",
                  backgroundColor: activeColumn === index ? "green" : "blueviolet",
                }}
              >
                <div
                  style={{
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "20px" }}>
          <thead>
            <tr>
              ...
              <th style={{ border: "1px solid black", padding: "8px" }}>ASIN</th>
          ...
          <tbody>
            {skuListData.map((skuItem, index) => (
              <tr key={index}>
                <td style={{ border: "1px solid black", padding: "8px" }}>{skuItem.sku}</td>
                <td style={{ border: "1px solid black", padding: "8px" }}>{skuItem.asin}</td>
                <td style={{ border: "1px solid black", padding: "8px", backgroundColor: "lightcyan", fontWeight: "bold" }}>{skuItem.amount + " $"}</td>
                <td style={{ border: "1px solid black", padding: "8px" }}>{skuItem.productName}</td>
              </tr>
            ))}
          </tbody>
        </table>
